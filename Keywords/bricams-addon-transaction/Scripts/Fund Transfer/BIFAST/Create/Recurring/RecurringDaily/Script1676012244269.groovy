...
try {
	if (WebUI.verifyElementPresent(findTestObject('Page_BRICaMS Cash  Trade/Fund Transfer/BIFAST/Create/ErrorISB'), 3)) {
		...
		int counter = 0
		while (!isElementFound && counter < 3) {
			try {
				WebUI.click(findTestObject('Page_BRICaMS Cash  Trade/Fund Transfer/BIFAST/Create/Button Check'))
				WebUI.click(findTestObject('Page_BRICaMS Cash  Trade/Fund Transfer/BIFAST/Create/button_Nextpage1'))
				isElementFound = true
		...
		// jika element tidak ditemukan, blok ini akan dieksekusi
		println('Element tidak ditemukan')
	}
} catch (Exception e) {
...
WebUI.setText(findTestObject('Page_BRICaMS Cash  Trade/Fund Transfer/BIFAST/Create/RecurringFrekuensi'), 'Daily' + Keys.ENTER)
...
String daily = dateFormat.format(cal.getTime())

WebUI.setText(findTestObject('Page_BRICaMS Cash  Trade/Fund Transfer/BIFAST/Create/input_End PeriodRecurring'), daily + 
    Keys.ENTER)
