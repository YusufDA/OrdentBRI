...
WebUI.navigateToUrl('https://addons.cms.dev.bri.co.id/external-transfer/remmitance/remmitance-transfer/create')
...
def tomorrowString = tomorrow.format(DateTimeFormatter.ofPattern('yyyy-MM-dd'))

WebUI.setText(findTestObject('Page_BRICaMS Cash  Trade/Fund Transfer/SWIFT/Create/Date'), tomorrowString + Keys.ENTER)

WebUI.setText(findTestObject('Page_BRICaMS Cash  Trade/Fund Transfer/SWIFT/Create/Date Jam'), '07:00' + Keys.ENTER)

WebUI.delay(3)

WebUI.setText(findTestObject('Page_BRICaMS Cash  Trade/Fund Transfer/SWIFT/Create/Input TF Purpose'), 'Family Support/Living Expenses' + 
    Keys.ENTER)
...
try {
    if (WebUI.verifyElementPresent(findTestObject('Page_BRICaMS Cash Trade/Fund Transfer/SWIFT/Create/Confirm Cutt off'), 
        3)) {
